...
function fillUsersConnect(userName){
console.log("fillUsersConnect");

usersConnect.push(userName);
connectWith();
}
function connectWith() {
console.log("fillUsersConnect");
//usersConnect.push('hani');
//usersConnect.push('duaa');

serverConnection.send(JSON.stringify({ 'displayName': localDisplayName, 'uuid': localUuid, 'dest': usersConnect,'type':'connect' }));
};
...
function gotMessageFromServer(message) {
  ...
  if (signal.displayName && signal.dest == localDisplayName) {
    console.log(signal.dest);
    // set up peer connection object for a newcomer peer
    if(typeof  peerConnections[peerUuid] == "undefined") {
    setUpPeer(peerUuid, signal.displayName);
    serverConnection.send(JSON.stringify({ 'displayName': localDisplayName, 'uuid': localUuid, 'dest': peerUuid ,'type':'continue'}));}

  } else if (signal.displayName && signal.dest == localUuid) {
    // initiate call if we are the newcomer peer
    setUpPeer(peerUuid, signal.displayName, true);

  } else if (signal.sdp&&signal.dest == localUuid) {
  ...
  } else if (signal.ice&&signal.dest == localUuid) {
...
function gotIceCandidate(event, peerUuid) {
  if (event.candidate != null) {
    serverConnection.send(JSON.stringify({ 'ice': event.candidate, 'uuid': localUuid, 'dest': peerUuid ,'type':'continue' }));
  }
}
...
function createdDescription(description, peerUuid) {
  ...
  peerConnections[peerUuid].pc.setLocalDescription(description).then(function () {
    serverConnection.send(JSON.stringify({ 'sdp': peerConnections[peerUuid].pc.localDescription, 'uuid': localUuid, 'dest': peerUuid ,'type':'continue' }));
  }).catch(errorHandler);
}
...
function makeButton(label) {
   ...
   vidButton.appendChild(document.createTextNode(label));
//  vidButton.onclick = fillUsersConnect(label);
//  vidLabel.setAttribute('class', 'videoLabel');
   vidButton.onclick= function(el) {
                          // disable your button immediately upon click
                          el.target.disabled = true;
                          fillUsersConnect(label);
                          }
  return vidButton;
}
...
function loadUsers(){
 ...
 xhttp.onload = function() {
        ...
        allUsersListData=jsonResponse
        console.log('usersListData', allUsersListData);
        showUserList(allUsersListData) ;
  }
...
function showUserList(allUsersListData){
//console.log(usersListData.length());
console.log('showUserList' , allUsersListData);
//usersListData.forEach(element => console.log(element));

for(let i=0; i<allUsersListData.length; i++){
console.log("i", i)
if(allUsersListData[i] !== localDisplayName){
 document.getElementById("usersList").appendChild(makeLabel(allUsersListData[i]));
 document.getElementById("usersList").appendChild(makeButton(allUsersListData[i]));
 console.log("typeof", typeof(allUsersListData))
}
console.log('this the name of the current user' ,allUsersListData[i])
